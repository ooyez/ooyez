<!--Header-->
  ![header](https://capsule-render.vercel.app/api?type=cylinder&color=F9D648&height=180&section=header&text=Welcome,%20YEZ’s%20GitHub&fontSize=45&fontColor=2E2E2E&fontAlignY=55)

<br />

## 🌼 About Me
안녕하세요! 프론트엔드와 앱 개발을 중심으로  
**사용자 친화적인 경험을 만드는 개발자 YEZ입니다.**  
UI/UX를 탐구하며 더 나은 인터페이스를 구현하는 데 즐거움을 느낍니다.

<br />

## 🛠 Languages
<div align="center">
<img src="https://img.shields.io/badge/Java-FEAA2B?style=flat-square&logoColor=white"/>
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=JavaScript&logoColor=white"/>
<img src="https://img.shields.io/badge/html5-E34F26?style=flat-square&logo=html5&logoColor=white"/>

### 🔧 Backend  
<img src="https://skillicons.dev/icons?i=spring,java" />

### 📱 App  
<img src="https://skillicons.dev/icons?i=flutter,dart" />

### 🗄 Database  
<img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=MySQL&logoColor=white"/>


### 🧰 Tools  
<img src="https://skillicons.dev/icons?i=git,github,figma,vscode,androidstudio" />

</div>

<br />

## 🚀 Projects

### 📦 화물 운송 플랫폼 (Team Project)
**Stack:** React, Spring Boot, MySQL, Flutter  
**Role:** 로그인, 회원가입, 마이페이지 일부 기능, 관리자 페이지 백엔드  
**Features:**  
- 화주/차주 구분 로그인  
- 실시간 배송 매칭 및 상태 관리  
- 관리자용 대시보드  

---

### 🎬 영화 예매 사이트 (Team Project)
**Stack:** HTML, CSS, JavaScript  
**Features:**  
- 영화 선택 및 예매  
- 로그인 / 회원가입  
- 상영 시간표 기반 예매 UI  

---

### 🥗 성인병 관리 웹사이트 (Personal Project)
**Stack:** React, Firebase (예정)  
**Features:**  
- 질병별 맞춤 식단 제공  
- 영양 정보 DB  
- 복약 시간 알림 기능  

<br />

## 📊 GitHub Stats
<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=yez&show_icons=true&theme=graywhite&title_color=F9D648&icon_color=F5C000" height="170" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=yez&layout=compact&theme=graywhite&title_color=F9D648" height="170" />

</div>

<br />

## 🌻 Contact
📩 **Email:** yourmail@example.com  
📎 포트폴리오 사이트·노션 링크 등도 여기에 추가할 수 있어요.
